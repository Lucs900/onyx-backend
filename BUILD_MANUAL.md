...
- Structure is a tappable term sheet, not a dashboard and not a File/Preview recap. Render only lines that exist: Path, Product, Occupancy, Timeline, Numbers (as given, never invented), Rate, Reward (ACR + after sketch only), Docs, Status. Paper / paper-elevated / ink / metal. Radius 16, hairline. No gold fill. Tap a line to fix it. Do not show a “Which part should Fox fix?” menu. Needs a correction → `Tap any line on the structure.`
...
- Disclosure on the thread: `ONYX Fox can assist and prepare. It cannot approve, lock, or commit to lend.` Fox cannot approve, lock, or commit to lend. California only stays site-level. Do not inherit the public Equity Fox chat.
...
- Docs are opportunistic before Confirm. Bubbles: `Upload now` / `Skip for now`. Skip writes `Skipped`. Missing docs do not block Confirm. No income wall. No Waiting/Add slot wall.
- Upload uses the compact `DocumentDrop` on the Structure card when the docs prompt is active (mobile in the Structure sheet). Skip does not block a draft.
