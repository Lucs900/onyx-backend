...
- **Workspace composer.** `.fox-stage--workspace .fox-bar__desk` is a bare line (no nested card, border, or lift). Fox mark + send stay. Do not restyle the homepage / dock Ask ONYX Fox bar.
- **Edits.** Prior client answers can show a quiet Edit control. Natural language (`change loan amount to 650,000`, `edit FICO`, occupancy/timeline/product/path/value/term) is parsed in `workspace.ts`. Updates go through `applyCapture`. Fox confirms briefly (`Updated loan amount to $650,000.`) and returns to the current prompt without restarting the file. Works after handoff; Fox still cannot approve, lock, or commit to lend.
