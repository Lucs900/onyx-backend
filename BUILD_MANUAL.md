...
- Fox flow (same on desktop and mobile): path (from homepage CTA) → product (Buy / Refinance / Use equity) → occupancy → timeline → loan amount → property value if needed → sample structure + `Does this look right?` → after Looks right, income type if unknown → docs only if income is W-2 / self-employed / both → handoff. Do not ask credit or term on this main line. The sample assumes Conventional 30-year at 6.750%. Use equity is a real HELOC/equity path (`heloc-heloan`). One question at a time, bubbles for structured answers, free text and composer always available. Money questions autofocus a text composer with a visible caret (`inputMode=decimal`, never `type=number`) and restore focus if it is lost. Not sure / Skip for now remain. Unknown or skip leaves the amount blank and continues. Pure numbers gain commas while typing (`6000` → `6,000`); `600000`, `600,000`, and `600k` confirm as `$600,000`. Free text is not force-formatted.
...
- After Looks right, if income is not known, Fox asks `How is income earned?` (W-2 / Self-employed / Both / Other). Docs come only after that confirm **and** a known W-2 / self-employed / both profile, filtered (paystubs / W-2; tax returns / business docs; or both). Bubbles: `Upload now` / `Skip for now`. Do not ask for docs when income is missing, empty, Other, or unknown. Other skips docs and continues to handoff.
