...
- Preview is a calm file card, not a dashboard. Show only real/available facts: path (ACR or Loan only), product (Buy / Refinance / Use equity), occupancy, timeline, rough loan amount or value, sample structure/rate/payment when shown, docs status, draft status. Temporary sample pricing: Conventional 30-year at 6.750%. Estimated monthly P&I uses loan amount + 6.750%/12 + 360 months (standard amortization). If loan amount is unknown, say so and still show the sample rate/term. Every price/payment/reward line is labeled `Sample · indicative · not live`. Estimated ACR reward range only on the ACR path via private `estimateFromDraft` (conservative sample-safe credit fallback if credit is missing). Never invent a live rate. Never show the private formula or a public %. Never imply approval, lock, or a commitment to lend.
- Fox flow (same on desktop and mobile): path (from homepage CTA) → product (Buy / Refinance / Use equity) → occupancy → timeline → loan amount → property value **only if Buy** → sample structure **in the chat thread** + `Does this look right?` → Looks right → handoff. Refinance / Use equity must not wait on property value. Preview updating is not the end of the turn. Do not ask credit, term, or income on this emergency path. Docs only after Looks right, and only if income is already known as W-2 / self-employed / both. If income is unknown, skip docs. The sample assumes Conventional 30-year at 6.750%. Use equity is a real HELOC/equity path (`heloc-heloan`). One question at a time, bubbles for structured answers, free text and composer always available. Money questions autofocus a text composer with a visible caret (`inputMode=decimal`, never `type=number`) and restore focus if it is lost. Not sure / Skip for now remain. Unknown or skip still continues to the sample card (`Loan amount not set`). Pure numbers gain commas while typing (`6000` → `6,000`); `600000`, `600,000`, and `600k` confirm as `$600,000`. Free text is not force-formatted.
...
- After Looks right, Fox hands off (`A licensed originator will review…`). Do not insert an income question. Docs come only after Looks right, and only if income is already known as W-2 / self-employed / both, filtered (paystubs / W-2; tax returns / business docs; or both). Bubbles: `Upload now` / `Skip for now`. If income is missing, empty, Other, or unknown, skip docs.
- Upload uses the compact `DocumentDrop` in the live preview (desktop on the File card when the docs prompt is active; mobile in the File / Preview sheet). Skip does not block a draft.
- After amounts, Fox writes the sample structure in chat (`Here’s a sample structure.`) — Conventional 30-year, sample rate 6.750%, estimated payment if amount is known, ACR reward range on the ACR path. Every price/payment/reward line is `Sample · indicative · not live`. Then Fox asks (exact): `Does this look right?` Bubbles: `Looks right` / `Needs a correction`. Confirm is not preview-only — the written summary lives in the thread on both layouts.
- **Looks right:** accept the sample, then income (if needed) and docs (if the profile is known). After those steps, draft status becomes **With originator**. Fox says a licensed originator will review the file and that Fox cannot approve, lock, or commit to lend. Fox stays for process questions. Do not swap to a human-chat skin.
- **Needs a correction** stays in Fox (`Which part should Fox fix?`). Product, occupancy, timeline, and amount can be corrected before the sample is accepted. After a fix, Fox restates the sample and asks again. Income and documents can be corrected after confirm.
...
- **Edits.** Prior client answers can show a quiet Edit control. Natural language (`change loan amount to 650,000`, `edit FICO`, occupancy/timeline/product/path/value/term/income) is parsed in `workspace.ts`. Updates go through `applyCapture`. Fox confirms briefly (`Updated loan amount to $650,000.`) and returns to the current prompt without restarting the file. Works after handoff; Fox still cannot approve, lock, or commit to lend.
- ACR and loan only stay distinct through this flow (path-specific starters and reward-panel rules). Reward estimate only on ACR via private `estimateFromDraft`. Temporary sample rate is 6.750% Conventional 30-year, labeled `Sample · indicative · not live`. No live rate. No public %. No disclosures in every bubble.
