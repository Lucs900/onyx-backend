...
## Add
One cell inside the File square on `/staff/hub`:

- Need — Last year’s W-2

Paint only from the still-useful / open-item line the File already stores on the notepad. If that line is not stored, show —. Do not write a new condition. Do not invent a second need.

## Do not
- Ask again (no new Fox question).
- Invent a second need.
- Touch `/start`.
- Move Collateral, Price, Borrower, Property, or Income, or change their values.
